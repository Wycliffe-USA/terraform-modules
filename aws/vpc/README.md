...
This is an opinionated Terraform module which creates VPC resources on AWS. It can be used to create a VPC with subnets for public, private, intra, database, elasticache and other subnets.  It also creates additional resources like network ACLs.
It can be used in a VPC that is segmented from other VPCs or in one that is attached to a transit gateway. This module is based on terraform-aws-modules/terraform-aws-vpc.

## Usage for stand-alone VPC

```hcl
module "vpc" {
  source = "github.com/Wycliffe-USA/aws/vpc"
  ...
  public_subnets                = {
    "10.11.3.0/24"   = { cidr = "10.11.3.0/24", az = "us-east-1a" }
    "10.11.4.0/24"   = { cidr = "10.11.4.0/24", az = "us-east-1b" }
    "10.11.5.0/24"   = { cidr = "10.11.5.0/24", az = "us-east-1c" }
  }
...
module "vpc" {
  ...
  azs                           = ["us-east-1a", "us-east-1b", "us-east-1c"]
  private_subnets  = {
    "10.11.0.0/24"   = { cidr = "10.11.0.0/24", az = "us-east-1a" }
    "10.11.1.0/24"   = { cidr = "10.11.1.0/24", az = "us-east-1b" }
    "10.11.2.0/24"   = { cidr = "10.11.2.0/24", az = "us-east-1c" }
  }
...
| <a name="input_intra_subnets"></a> [intra\_subnets](#input\_intra\_subnets) | A map of maps contianing intra subnets inside the VPC. Expected values are are cidr and az. Valid values also include name, and tags. Name overrides the default subnet naming. | `map(any)` | `{}` | no |
...
| <a name="input_name"></a> [name](#input\_name) | Name to be used on all the resources as identifier. Overrides app_name & app_env. | `string` | `""` | no |
...
Module is maintained by [Dave Barnum](https://github.com/barnumd).

## Original Authors

Original VPC Module is maintained by [Anton Babenko](https://github.com/antonbabenko) with help from [these awesome contributors](https://github.com/terraform-aws-modules/terraform-aws-vpc/graphs/contributors).
